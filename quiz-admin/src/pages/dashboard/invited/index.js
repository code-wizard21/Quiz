...
import { getInvitedUsers } from "../../../api/user.service";
...
const Customers = () => {
  ...
  const getCustomers = useCallback(async () => {
    ...
    try {
      const result = await getInvitedUsers(controller.page + 1, 10, "desc", "createdAt:desc");
      if (isMounted()) {
        setCustomersState(() => ({
          isLoading: false,
  ...
  return (
    <>
      <Head>
        <title>Customer: List | Lobang Dashboard</title>
      </Head>
      <Box sx={{ flexGrow: 1 }}>
        <Container
          maxWidth="xl"
